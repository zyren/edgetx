...
{
  HAL_StatusTypeDef status = HAL_OK;
  uint32_t tmp = hdma->Init.DMA_FIFOThreshold;

  /* Memory Data size equal to Byte */
  if(hdma->Init.DMA_MemoryDataSize == DMA_MDATAALIGN_BYTE)
  {
    switch (tmp)
    {
    case DMA_FIFO_THRESHOLD_1QUARTERFULL:
    case DMA_FIFO_THRESHOLD_3QUARTERSFULL:
      if ((hdma->Init.DMA_MemoryBurst & DMA_SxCR_MBURST_1) == DMA_SxCR_MBURST_1)
      {
        status = HAL_ERROR;
      }
      break;
    case DMA_FIFO_THRESHOLD_HALFFULL:
      if (hdma->Init.DMA_MemoryBurst == DMA_MBURST_INC16)
      {
        status = HAL_ERROR;
      }
      break;
  ...
  else if (hdma->Init.DMA_MemoryDataSize == DMA_MDATAALIGN_HALFWORD)
  {
    switch (tmp)
    {
    case DMA_FIFO_THRESHOLD_1QUARTERFULL:
...
HAL_StatusTypeDef HAL_DMA_Init(DMA_HandleTypeDef *hdma)
{
  ...
  if(hdma->Init.DMA_FIFOMode != DMA_FIFOMODE_DISABLE)
  {
  ...
  {
    /* Check for the Timeout */
    if((xTaskGetTickCount() - tickstart ) > HAL_TIMEOUT_DMA_ABORT)
    {
      /* Update error code */
  ...
  tmp |=  hdma->Init.DMA_Channel            | hdma->Init.DMA_DIR            |
          hdma->Init.DMA_PeripheralInc      | hdma->Init.DMA_MemoryInc      |
          hdma->Init.DMA_PeripheralDataSize | hdma->Init.DMA_MemoryDataSize |
          hdma->Init.DMA_Mode               | hdma->Init.DMA_Priority;

  /* the Memory burst and peripheral burst are not used when the FIFO is disabled */
  if(hdma->Init.DMA_FIFOMode == DMA_FIFOMODE_ENABLE)
  {
    /* Get memory burst and peripheral burst */
    tmp |=  hdma->Init.DMA_MemoryBurst | hdma->Init.DMA_PeripheralBurst;
  }
  ...
  tmp |= hdma->Init.DMA_FIFOMode;

  /* The FIFO threshold is not used when the FIFO mode is disabled */
  if(hdma->Init.DMA_FIFOMode == DMA_FIFOMODE_ENABLE)
  {
    /* Get the FIFO threshold */
    tmp |= hdma->Init.DMA_FIFOThreshold;

    /* Check compatibility between FIFO threshold level and size of the memory burst */
    /* for INCR4, INCR8, INCR16 bursts */
    if (hdma->Init.DMA_MemoryBurst != DMA_MBURST_SINGLE)
    {

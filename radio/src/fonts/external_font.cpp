...
ExternalFontResult Manager::prepare(const Reader & reader)
{
  ...
  const uint32_t payloadOffset = get32(ioBuffer + 28);
  const uint32_t payloadLength = get32(ioBuffer + 32);
  ...
  if (!addOk(dirOffset, directoryLength, directoryEnd) ||
      !addOk(payloadOffset, payloadLength, payloadEnd) ||
      payloadOffset != expectedPayloadOffset || payloadOffset < directoryEnd ||
      payloadLength != uint32_t(strikeCount_) * kStrikeLength || payloadEnd != fileSize_)
    return unavailable();
  ...
  for (uint32_t pos = directoryEnd; pos < payloadOffset;) {
    const uint32_t amount = payloadOffset - pos > sizeof(ioBuffer) ?
                            sizeof(ioBuffer) : payloadOffset - pos;
  ...
  uint32_t expected = payloadOffset;
  for (uint8_t i = 0; i < strikeCount_; ++i)
...
bool Manager::activate()
{
  ...
  const FRESULT result = reader_.createLinkMap(reader_.context);
...
FRESULT fatLinkMap(void * context)
{
  FIL * file = static_cast<FIL *>(context);
#if FF_USE_FASTSEEK
  clusterMap[0] = external_font::kLinkMapWords;
  file->cltbl = clusterMap;
  const FRESULT result = f_lseek(file, CREATE_LINKMAP);
  if (result != FR_OK) file->cltbl = nullptr;
  return result;
#else
  (void)file;
